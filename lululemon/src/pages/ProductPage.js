...
export const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${singleProductURL}/${productID}?mykey=${myKey}`)
      .then((res) => {
        ...
        if (colorId) {
          ...
          const swatchIndex = productData.swatches.findIndex(
  ...
  const getCurrentImagesAndAlts = () => {
    const currentImages = product.images?.find(
      (image) => image.colorId === selectedColorId,
    );
    if (currentImages) {
      return {
        images: currentImages.mainCarousel.media
          .split("|")
          .map((img) => img.trim()),
  ...
  const handleAddToBag = () => {
    ...
    const productPrice = Number(
      parseInt(newPrice.slice(0, newPrice.indexOf(" "))),
    );
    const cartItem = {
      productId: productID,
    ...
    // check if user is logged in, if YES, store cartItem in db, if NO, put it in localStorage to render
    if (isLoggedIn) {
      const cartId = localStorage.getItem("cartId");
      dispatch(addItemToServer(cartItem, cartId));
    } else {
  ...
  const fetchRecommendedProducts = async (colorId) => {
    try {
      ...
      const filteredProducts = products
        .filter((product) =>
          product.swatches.some((swatch) => swatch.colorId === colorId),
        )
        .slice(0, 4);
  ...
  return (
    <>
      <div
        className={
          isModalVisible === true
            ? "productPageWrapperHidden"
            : "productPageWrapperActive"
        }
      >
        <Header />
        <div className="productPageContainer">
          <div className="productInfoContainer">
            ...
            <div className="productInfo">
              ...
              <ProductDetails
                product={product}
                refs={refs}
                handleScroll={handleScrollAndExpand}
              />
            </div>
          ...
          {product.whyWeMadeThis && (
            <WhyWeMadeThis
              ...
              setExpendedIndex={setExpendedIndex}
            />
          )}
        </div>
