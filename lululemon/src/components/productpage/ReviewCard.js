...
export const ReviewCard = ({index, date, helpful, size, name, rating, title, comment}) => {
    ...
    return <div className="reviewCard">
        ...
        <div className="reviewCardRating">
            {stars.length > 0 && stars.map(item => {
                if (item === 1) {
                    return <img src={star_10} alt=""/>
                } else if (item <= 0.3) {
                    return <img src={star_1_3} alt=""/>
                } else if (item <= 0.5) {
                    return <img src={star_4_5} alt=""/>
                } else if (item <= 0.8) {
                    return <img src={star_6_8} alt=""/>
                } else {
                    return <img src={star_10} alt=""/>
                }
            })}
        ...
        <div className="reviewCardSize">
            {size &&
                <div className="reviewCardSizeContainer">
                    <div className="reviewCardSizeTitle">
                        Size Purchased:
                    </div>
                    <div className="reviewCardSizeNum">
                        {size}
                    </div>
                </div>}
            <div className="reviewCardSizeContainer">
            ...
            <div className="reviewReactionHelpful">
                {helpCount > 0
                    ? <div className="helpful">
                        <img onClick={() => {
                            dispatch(addToHelpful(index))}} src={like_solid} alt="thumb"/>
                        {helpCount}
                    </div>
                    : <div className="helpful">
                        <img onClick={() => {dispatch(addToHelpful(index))}} src={like_outline} alt=""/>
                    </div>}
            </div>
