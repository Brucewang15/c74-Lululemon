...
export const LoginModal = ({handleModalClose, isSuccess, setIsSuccess}) => {
    ...
    const handleSignIn = () => {
        axios.post(`http://api-lulu.hibitbyte.com/auth/login?mykey=${myKey}`, {
            email,
            password
        })
            .then(res => {
    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                ...
                {message && <p className={isSuccess === true ? 'loginSuccess' : 'loginFail'}>{message}</p>}
                <form onSubmit={handleSubmit}>
                    <div className="input-group">
                        <label htmlFor="email">Email address</label>
                        <input type="email" id="email" name="email" onChange={(e) => handleEmailInput(e)}
                               required/>
                    </div>
                    <div className="input-group">
