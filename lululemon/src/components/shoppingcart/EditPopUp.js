...
export const EditPopUp = ({
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      const swatch = fetchedProduct.swatches.find(
  ...
  const handleUpdateBag = (itemId, index, size, colorId, colorDes, image) => {
    if (isLogin === true) {
      ...
      dispatch(editServerCart(cartId, itemId, newItem));
    } else {
      console.log("HANDLE UPDATE BAG CURRENT IMAGE", currentImage);
      console.log(image);
      dispatch(editCart(size, colorId, index, colorDes, image));
    ...
    <div className="editPopUp">
      <div className="productEdit">
        ...
        <div className="detailEdit">
          ...
          <div className="closeOutBtn" onClick={() => closeOut(index)}>
            x
          </div>
        </div>
      </div>
