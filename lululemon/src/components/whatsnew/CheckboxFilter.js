...
export const CheckboxFilter = ({ filterType, filters }) => {
  ...
  const filterViewMore = useSelector(
    (state) => state.filterReducer.filterViewMore,
  );
  ...
  return (
    <div className="checkboxFilter">
      ...
      >
        <div className={isVisible ? "filterTypeNameBold" : "filterTypeName"}>
          {filterType}
        </div>
      ...
      {filters[filterType]
        .slice(
          0,
          filterViewMore[filterType] ? filterViewMore[filterType].length : 5,
        )
        .map(
          (filter, index) =>
            isVisible && (
              <div
                className={`filterDetailsBox ${isVisible ? "fadeIn" : "fadeOut"}`}
                key={filter.id || index}
              >
      ...
      {isVisible && filters[filterType].length > 5 && (
        <div
